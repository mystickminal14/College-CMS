...
import useChangeShiftOrder from "../hooks/useChangeShiftOrder";
...
const ChangeShiftOrderModal: React.FC<Props> = ({ isOpen, onClose, shift, maxOrder }) => {
  const [newOrder, setNewOrder] = useState<number>(shift?.order ?? 1);

  const changeOrderMutation = useChangeShiftOrder();

  useEffect(() => {
    if (isOpen && shift) {
      setNewOrder(shift.order ?? 1);
    }
  }, [isOpen, shift]);
        ...
        >
          ...
          <div className="bg-[#1a7cd3] p-6 flex justify-between items-center">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-white">Change Shift Order</h2>
                <p className="text-white/80 text-sm mt-1">
                  Update the display order for <span className="font-semibold">{shift.name}</span>
                </p>
              </div>
            </div>
            <button
...
export default ChangeShiftOrderModal;

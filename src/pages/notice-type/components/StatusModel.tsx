...
import type { ENoticeTypeStatus, NoticeType } from "../model/NoticeTypeModel";
...
interface Props {
  ...
  mutation: UseMutationResult<
    ...
    ApiErrorResponse,
    { id: number; status: ENoticeTypeStatus }
  >;
}

const StatusModal = ({ isOpen, onClose, data, mutation }: Props) => {
  const [status, setStatus] = useState<ENoticeTypeStatus>("DISABLED");
    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center">
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-xl w-full max-w-md p-6">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <select
            value={status}
            onChange={(e) => setStatus(e.target.value as ENoticeTypeStatus)}

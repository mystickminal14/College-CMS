...
import useChangeNoticeTypeOrder from "../hooks/useChangeNoticeTypeOrder";
...
const ChangeNoticeTypeOrderModal: React.FC<Props> = ({
  isOpen,
  onClose,
  noticeType,
  maxOrder,
}) => {
  const [newOrder, setNewOrder] = useState<number>(noticeType?.order ?? 1);

  const changeOrderMutation = useChangeNoticeTypeOrder();

  useEffect(() => {
    if (isOpen && noticeType) {
      setNewOrder(noticeType.order ?? 1);
    }
  }, [isOpen, noticeType]);
        ...
        >
          ...
          <div className="bg-[#1a7cd3] p-6 flex justify-between items-center">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-white">Change Notice Type Order</h2>
                <p className="text-white/80 text-sm mt-1">
                  Update the display order for{" "}
                  <span className="font-semibold">{noticeType.name}</span>
                </p>
              </div>
            </div>
            <button
...
export default ChangeNoticeTypeOrderModal;

...
import useChangeCourseCategoryOrder from "../hooks/useChangeOrder";

interface Props {
  isOpen: boolean;
  onClose: () => void;
  coursecategory?: CourseCategory | null;
  maxOrder?: number; // maximum order allowed
}

const ChangeCourseCategoryOrderModal: React.FC<Props> = ({ isOpen, onClose, coursecategory, maxOrder }) => {
  const [newOrder, setNewOrder] = useState<number>(coursecategory?.order ?? 1);

  const changeOrderMutation = useChangeCourseCategoryOrder();

  useEffect(() => {
    if (isOpen && coursecategory) {
      setNewOrder(coursecategory.order ?? 1);
    }
  }, [isOpen, coursecategory]);

  if (!isOpen || !coursecategory) return null;
        ...
        >
          ...
          <div className="bg-[#1a7cd3] p-6 flex justify-between items-center">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-white">Change CourseCategory Order</h2>
                <p className="text-white/80 text-sm mt-1">
                  Update the display order for <span className="font-semibold">{coursecategory.name}</span>
                </p>
              </div>
            </div>
            <button
...
export default ChangeCourseCategoryOrderModal;

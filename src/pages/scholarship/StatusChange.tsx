...
import type { ScholarshipSchedule, ScholarStatus } from "./model";
...
const StatusModal = ({ isOpen, onClose, data, mutation }: Props) => {
  const [status, setStatus] = useState<ScholarStatus>("CLOSED");

  useEffect(() => {
    if (data) setStatus(data.status || "CLOSED");
  }, [data]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    mutation.mutate({ id: data.id, status }, { onSuccess: () => onClose() });
  };
    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center">
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-xl w-full max-w-md p-6">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <select
            value={status}
            onChange={(e) => setStatus(e.target.value as ScholarStatus)}
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            >
              {mutation.isLoading ? "Saving..." : "Update Status"}
            </button>
          </div>
        </form>
      </div>

...
const ChangedownloadOrderModal: React.FC<Props> = ({ isOpen, onClose, download, maxOrder }) => {
  const [newOrder, setNewOrder] = useState<number>(download?.order ?? 1);

  const changeOrderMutation = useChangeOrder();

  useEffect(() => {
    if (isOpen && download) {
      setNewOrder(download.order ?? 1);
    }
  }, [isOpen, download]);
        ...
        >
          <div className="bg-[#1a7cd3] p-6 flex justify-between items-center">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-white">Change download Order</h2>
                <p className="text-white/80 text-sm mt-1">
                  Update the display order for <span className="font-semibold">{download.name}</span>
                </p>
              </div>
            </div>
            <button
...
export default ChangedownloadOrderModal;

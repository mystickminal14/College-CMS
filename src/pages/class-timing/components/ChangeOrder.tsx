...
import useChangeClassTimingOrder from "../hooks/useChangeClassTimingOrder";
...
const ChangeClassTimingOrderModal: React.FC<Props> = ({
  isOpen,
  onClose,
  classTiming,
  maxOrder,
}) => {
  const [newOrder, setNewOrder] = useState<number>(classTiming?.order ?? 1);

  const changeOrderMutation = useChangeClassTimingOrder();

  useEffect(() => {
    if (isOpen && classTiming) {
      setNewOrder(classTiming.order ?? 1);
    }
  }, [isOpen, classTiming]);
        ...
        >
          ...
          <div className="bg-[#1a7cd3] p-6 flex justify-between items-center">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-white">Change Class Timing Order</h2>
                <p className="text-white/80 text-sm mt-1">
                  Update the display order for{" "}
                  <span className="font-semibold">{classTiming.name}</span>
                </p>
              </div>
            </div>
            <button
...
export default ChangeClassTimingOrderModal;

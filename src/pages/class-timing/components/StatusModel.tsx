...
import type { EClassTimingStatus, ClassTiming } from "../model/ClassTimingModel";
import type { ApiErrorResponse, ApiResponse } from "../../../services/apiTypes";

interface Props {
  isOpen: boolean;
  onClose: () => void;
  data: ClassTiming | null;
  mutation: UseMutationResult<
    ApiResponse<ClassTiming>,
    ApiErrorResponse,
    { id: number; status: EClassTimingStatus }
  >;
}

const StatusModal = ({ isOpen, onClose, data, mutation }: Props) => {
  const [status, setStatus] = useState<EClassTimingStatus>("DISABLED");
    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center">
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-xl w-full max-w-md p-6">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <select
            value={status}
            onChange={(e) => setStatus(e.target.value as EClassTimingStatus)}

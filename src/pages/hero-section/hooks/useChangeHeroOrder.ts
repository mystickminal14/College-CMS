import { useMutation, useQueryClient } from "@tanstack/react-query";
import { useContext } from "react";
import { AppContext } from "../../../context/ContextApp";
import type { ApiErrorResponse, ApiResponse } from "../../../services/apiTypes";
import { HERO_SECTION_CACHE_KEY } from "../../../constants";
import type { HeroSectionImage } from "../model/HeroModel";
import APIClient from "../../../services/apiClient";

interface ChangeOrderPayload {
  id: number;
  newOrder: number;
}

const useChangeHeroOrder = () => {
  const appContext = useContext(AppContext);
  if (!appContext) throw new Error("useChangeHeroOrder must be used within AppContext");
  const { showToast } = appContext;
  const queryClient = useQueryClient();

  return useMutation<ApiResponse<HeroSectionImage>, ApiErrorResponse, ChangeOrderPayload>({
    mutationFn: ({ id, newOrder }) => {
      const apiClient = new APIClient<HeroSectionImage>(
        `/hero_section_image/change-order/${encodeURIComponent(id)}`
      );
      return apiClient.put({ newOrder });
    },
    onSuccess: (res) => {
      showToast(res.message || "Order updated successfully!", "success");
      queryClient.invalidateQueries({ queryKey: [HERO_SECTION_CACHE_KEY] });
    },
    onError: (err) => {
      const msg = err.errors?.[0]?.message || err.message || "Failed to update order";
      showToast(msg, "error");
    },
  });
};

export default useChangeHeroOrder;
...
import useChangeDeptOrder from "../hooks/useChangeOrder";
...
const ChangeDeptOrderModal: React.FC<Props> = ({ isOpen, onClose, dept, maxOrder }) => {
  const [newOrder, setNewOrder] = useState<number>(dept?.order ?? 1);

  const changeOrderMutation = useChangeDeptOrder();

  useEffect(() => {
    if (isOpen && dept) {
      setNewOrder(dept.order ?? 1);
    }
  }, [isOpen, dept]);
        ...
        >
          ...
          <div className="bg-[#1a7cd3] p-6 flex justify-between items-center">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-white">Change Dept Order</h2>
                <p className="text-white/80 text-sm mt-1">
                  Update the display order for <span className="font-semibold">{dept.name}</span>
                </p>
              </div>
            </div>
            <button
...
export default ChangeDeptOrderModal;

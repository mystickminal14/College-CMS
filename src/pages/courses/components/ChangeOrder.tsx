...
import useChangeCourseOrder from "../hooks/useChangeOrder";
...
const ChangeCourseOrderModal: React.FC<Props> = ({ isOpen, onClose, course, maxOrder }) => {
  const [newOrder, setNewOrder] = useState<number>(course?.order ?? 1);

  const changeOrderMutation = useChangeCourseOrder();

  useEffect(() => {
    if (isOpen && course) {
      setNewOrder(course.order ?? 1);
    }
  }, [isOpen, course]);
        ...
        >
          ...
          <div className="bg-[#1a7cd3] p-6 flex justify-between items-center">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-white">Change Course Order</h2>
                <p className="text-white/80 text-sm mt-1">
                  Update the display order for <span className="font-semibold">{course.title}</span>
                </p>
              </div>
            </div>
            <button

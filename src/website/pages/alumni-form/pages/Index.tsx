...
import { EPrefix, EStudyMode, EStatus, type AlumniFormData } from '../models/alumniModel';
...
const AlumniFormPage: React.FC = () => {
  const [formData, setFormData] = useState<AlumniFormData>({
    ...
    designation: '',
    presentCountry: '',
    status: EStatus.ENABLED, // ✅ Added status with default
  });
  ...
  const validateForm = (): FormErrors => {
    ...
    // ✅ Validate prefix
    if (!formData.prefix || !Object.values(EPrefix).includes(formData.prefix)) {
      newErrors.prefix = 'Please select a valid prefix';
    }

    // ✅ Validate study mode
    if (!formData.mode || !Object.values(EStudyMode).includes(formData.mode)) {
      newErrors.mode = 'Please select a valid study mode';
    }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const submissionData: AlumniFormData = {
      ...formData,
      status: formData.status || EStatus.ENABLED,
      registrationDate: formData.registrationDate || new Date().toISOString(),
    };

    createAlumniMutation.mutate(submissionData, {
      onSuccess: () => {
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={handleChange}
                    className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition ${errors.prefix ? 'border-red-300' : 'border-gray-300'
                      }`}
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                ...
                <div>
                  ...
                  <select
                    ...
                    value={formData.mode}
                    onChange={handleChange}
                    className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition ${errors.mode ? 'border-red-300' : 'border-gray-300'
                      }`}
                  >
